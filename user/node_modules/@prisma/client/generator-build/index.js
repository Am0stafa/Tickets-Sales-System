...
${(0, import_indent_string9.default)(
        outputType.fields.filter((field) => field.outputType.location === "scalar" || field.outputType.location === "enumTypes").map((f2) => `${f2.name}?: boolean`).join("\n"),
        TAB_SIZE
      )}
}`;
    }, "")}
...
function clientExtensionsQueryDefinition() {
  ...
  const allOperationsParam = /* @__PURE__ */ __name((modelNames2, indent15) => {
    ...
    return `{
    ${indent15}$allOperations?: ${queryCbDefinition(key, `keyof Prisma.TypeMap<ExtArgs>[${key}]`)}
  ${indent15}}`;
  }, "allOperationsParam");
